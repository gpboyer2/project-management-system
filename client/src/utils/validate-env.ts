 ...
 *
 * 使用方法：
 * import { validateRequiredEnv } from '@/utils/validate-env';
 * validateRequiredEnv(import.meta.env, ['VITE_API_PORT', 'VITE_API_HOST'], '.env');
 */

/**
 * 校验必需的环境变量
 * @param envObj - 环境变量对象（如 import.meta.env 或 loadEnv 返回值）
 * @param requiredVars - 必需的环境变量名数组
 * @param envFileName - 环境变量文件名，用于错误提示
 * @param context - 额外的上下文信息（如 "Vite 配置"）
 * @throws 当有必需的环境变量缺失时
 */
