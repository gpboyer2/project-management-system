...
-- 为超级管理员分配新增的项目管理权限
INSERT OR IGNORE INTO ROLE_PERMISSIONS (ROLE_ID, PERMISSION_ID)
SELECT 1, PERMISSION_ID FROM PERMISSIONS WHERE PERMISSION_CODE LIKE 'REQUIREMENT:%' OR PERMISSION_CODE LIKE 'TASK:%' OR PERMISSION_CODE LIKE 'DATA_IMPORT_EXPORT:%' OR PERMISSION_CODE LIKE 'REQUIREMENT_VERSION:%';

-- 为管理员分配项目管理权限
INSERT OR IGNORE INTO ROLE_PERMISSIONS (ROLE_ID, PERMISSION_ID)
SELECT 2, PERMISSION_ID FROM PERMISSIONS WHERE (PERMISSION_CODE LIKE 'REQUIREMENT:%' OR PERMISSION_CODE LIKE 'TASK:%' OR PERMISSION_CODE LIKE 'DATA_IMPORT_EXPORT:%' OR PERMISSION_CODE LIKE 'REQUIREMENT_VERSION:%');

-- 为普通用户分配项目管理查看权限
INSERT OR IGNORE INTO ROLE_PERMISSIONS (ROLE_ID, PERMISSION_ID)
SELECT 3, PERMISSION_ID FROM PERMISSIONS WHERE PERMISSION_CODE IN (
    'REQUIREMENT:VIEW', 'TASK:VIEW', 'DATA_IMPORT_EXPORT:EXPORT', 'REQUIREMENT_VERSION:VIEW'
);
